...
def resize(img, **kwargs):
    ...
    if mode == 64:
        temp = np.zeros((256, 256, tl, zl))
        temp[:, :232, :, :] = img[16:, :232, :, :zl]

        ret = np.zeros((64,64,tl,zl))
        for t in range(tl):
            for z in range(zl):
...
def c2bmask(img, **kwargs):
    ...
    fig = plt.figure()
    ax1 = fig.add_subplot(1, 2, 1)
    ax1.imshow(ret[:, :], cmap=plt.cm.gray)
...
if __name__ == '__main__':
    ...
    img_original = np.load('cinedata/cinedata_1.npy')
...
# save 64*64 downsampled image & save 256*256 image

    np.save("img_64", img_64)
    np.save("img_256", img_256)

# manually crop & save circular mask

    cmask = img2mask(img_256[:, :, tslice, zslice], mask=None, position=None)
    np.save("circular_mask", cmask)

# derive center from the mask & save 96*96 mask centered at 'the center'

    bmask = c2bmask(cmask)
    center = get_center(cmask)
    # print(center)



    np.save("boxed_mask", bmask)

# downsample the mask to 32*32 & save

    temp = scipy.misc.imresize(bmask[:, :], (32, 32), interp = 'nearest')
    bmask_32 = np.zeros((32,32))
    for x in range(32):
        for y in range(32):
            if temp[x][y] >= 1:
                bmask_32[x][y] = 1

    np.save("boxed_mask_32", bmask_32)

    fig = plt.figure()
    ax1 = fig.add_subplot(1, 5, 1)
    ax1.imshow(img_256[:, :, tslice, zslice], cmap=plt.cm.gray, interpolation='none')
    """preventing plot from rescaling image:"""
    ax1.set_xlim([0.0, img_256.shape[1]])
    ax1.set_ylim([img_256.shape[0], 0.0])
    ax1.autoscale = False

    """preventing plot from clearing image:"""
    ax1.hold(True)
    ax1.plot(center[1], center[0], marker='o', markerfacecolor='b', linestyle='none', markersize=5)

    ax2 = fig.add_subplot(1, 5, 2)
    ax2.imshow(img_64[:, :, tslice, zslice], cmap=plt.cm.gray, interpolation='none')

    ax3 = fig.add_subplot(1, 5, 3)
    ax3.imshow(cmask[:, :], cmap=plt.cm.gray, interpolation='none')

    ax4 = fig.add_subplot(1, 5, 4)
    ax4.imshow(bmask[:, :], cmap=plt.cm.gray, interpolation='none')

    ax5 = fig.add_subplot(1, 5, 5)
    ax5.imshow(bmask_32[:, :], cmap=plt.cm.gray, interpolation='none')

    plt.show()


'''
    fname1 = 'test_perf.mat'
    mat_content = sio.loadmat(fname1)

    img = mat_content['img_myo']
    mask = mat_content['mask_epic']
    position = [(250,250), (180,180), (200, 200)]

    mask1 = img2mask(img, mask = mask).ret_mask()
    mask2 = img2mask(img, mask = mask1).ret_mask()
'''